...
+ (void)addArguments4ManualSpecifyNetworkServices:(NSMutableArray*) args {
    ...
    if (![defaults boolForKey:[Configuration C_AUTO_CONFIGURE_NETWORK_SERVICES]]) {
        NSArray* serviceKeys = [defaults arrayForKey:[Configuration C_PROXY4_NETWORK_SERVICES]];
        if (serviceKeys) {
...
+ (void)enableGlobalProxy {
    NSUInteger port = [[NSUserDefaults standardUserDefaults]integerForKey:[Configuration C_LOCAL_SOCKS5_LISTEN_PORT]];
    
    NSMutableArray* args = [@[@"--mode", @"global", @"--port"
                              , [NSString stringWithFormat:@"%lu", (unsigned long)port]]mutableCopy];
    
    if ([[NSUserDefaults standardUserDefaults] boolForKey:[Configuration C_LOCAL_HTTP_ON]] && [[NSUserDefaults standardUserDefaults] boolForKey:[Configuration C_LOCAL_HTTP_FOLLOW_GLOBAL]]) {
        NSUInteger privoxyPort = [[NSUserDefaults standardUserDefaults]integerForKey:[Configuration C_LOCAL_HTTP_LISTEN_PORT]];
...
+ (NSString*)startPACServer:(NSString*) PACFilePath {
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString * address = [defaults stringForKey:[Configuration C_PAC_SERVER_LISTEN_ADDRESS]];
    int port = (short)[defaults integerForKey:[Configuration C_PAC_SERVER_LISTEN_PORT]];
